defmodule GarageDetector.MotionServer do
  use GenServer
  alias Circuits.GPIO
  require Logger
  ...
  @impl true
  def init(_) do
    {:ok, trig} = GPIO.open(Application.get_env(:garage_detector, :hcsr04_trig_pin), :output)
    {:ok, echo} = GPIO.open(Application.get_env(:garage_detector, :hcsr04_echo_pin), :input)
    schedule_collection()
  ...
  def handle_info(:collect, %{trig: trig, echo: echo}) do
    ...
    cond do
      distance < 750 ->
        Tortoise.publish("garage_detector", "garage/car", "1", qos: 0)
        Logger.info("Distance: #{distance}cm")
        schedule_collection(20_000)
      true ->
        Tortoise.publish("garage_detector", "garage/car", "0", qos: 0)
        Logger.info("Distance: #{distance}cm")
        schedule_collection()
    end
